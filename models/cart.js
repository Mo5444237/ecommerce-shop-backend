...
cartSchema.methods.addToCart = function (productId, quantity, color, size) {
    ...
    if (
        productIndex >= 0 &&
        updatedCartItems[productIndex].color === color &&
        updatedCartItems[productIndex].size === size
    ) {
...
cartSchema.methods.decreaseFromCart = function (cartItemId) {
    const itemToUpdateIndex = this.items.findIndex(item => item._id.toString() === cartItemId.toString());
